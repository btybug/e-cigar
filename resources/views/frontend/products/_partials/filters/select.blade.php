<div class="form-group row select_with-tag-wrapper position-relative">
    ...
    <div class="simple_select_wrapper col-sm-8 p-sm-0">
        ...
        @endphp
        {!! Form::select("select_filter[$filter->id]",['' => "Select"] + $dataOptions,null,['class' => 'select-filter select-2 select-2--no-search main-select main-select-2arrows not-selected arrow-dark','id' => 'deliverySelect'.$filter->id] ) !!}
    </div>
</div>