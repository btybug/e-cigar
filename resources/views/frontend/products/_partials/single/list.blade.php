            ...
            <div class="col-sm-10 align-self-center">
              <div class="row justify-content-between product-list-item">
                <div class="align-self-center checkbox-wrap">
                  <input type="radio" id="pv{{ $item->id }}" data-id="" class="custom-control-input package_checkbox"
                         name="package_v[{{ $vape->id }}][]"  value="{{ $item->id }}" >
                  <label data-out="{{ out_of_stock($item) }}" class="product-single-info_radio-label custom-control-label font-15 text-gray-clr pointer package_radio_label" for="pv{{ $item->id }}">
                    {{ $item->name }}
