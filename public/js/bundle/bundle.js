...
$(document).ready(function () {
    ...
    // let heightLastProduct = $('body').find('.products__all-list-product >li').last().outerHeight();
    // $('.products__all-list-product').append('<li class="list--hidden-product"></li>');
    // if($('body').find('.list--hidden-product')){
    //     $('body').find('.list--hidden-product').css({height:heightLastProduct})
    // }
    //
    // product hover
    $('body').on('mouseover', '.products__item-wrapper', function () {
        if (!$(this).hasClass("product_list")) {
            var photoHeight = $(this).find('.products__item-photo-brand-name').outerHeight();
            var contentInnerHeight = $(this).find('.products__item-content-inner').outerHeight();
            var itemDescHeight = $(this).find('.products__item-desc').outerHeight();
            var contentHeight = contentInnerHeight - itemDescHeight - 3;
    ...
    $("#singleProductPageCnt").fadeIn(function () {
        ...
        var countOfferTotalPrice = function countOfferTotalPrice() {
            ...
            $('.added-offers').find('.special__popup-content-right-product-price').each(function (key) {
                offer_total_price += Number(Number($(this).data('price')).toFixed(2));
            });
        ...
        function activate_item(self, id, name, group_id, discount) {
            var limit = $(".product__single-item-info[data-group-id=\"" + group_id + "\"]").data('limit');
            if (limit !== 1) {
                if ($(self).hasClass('active')) {
                    ...
                    if (discount) {
                        var count = 0;
                        var currentCount = $(self).find('.product-qty-select').val() * 1;
                        $("#wizardViewModal #myTabContent #wizardAll").find('.item-content.active').each(function () {
                            count += $(this).find('.product-qty-select').val() * 1;
                        });
                        console.log('count -> ', count);
                        if (count + currentCount <= group_element.data('limit')) {
        ...
        $('body').on('click', '#wizardViewModal .item-content .discount_modal_count_js', function () {
            ...
            if (type === 'minus') {
                ...
                if (isActive) {
                    ...
                    console.table({ count: count, currentValue: currentValue });
                    if (count + 1 <= limit) {
            ...
            console.log($(this).siblings('.product-qty-select').val());
        });
        ...
        $('body').on('change', '.product__single-item-info.filter_discount .discount-radio', function (ev) {
            ...
            $(ev.target).closest('.filter_discount').find('.product-single-info_row-items').children().each(function (number, el) {
                if (number >= limit) {
                    $(el).remove();
                }
                                ...
                                if (!$("#wizardViewModal").data('discount')) {
                                    ...
                                    $("#wizardViewModal ul.content li").each(function () {
                                        $(this).find(".add_discount_js").on('click', function () {
                                            var id = $(this).closest('li').attr('data-id');
                                            var name = $(this).closest('li').attr('data-name');
                                            activate_item($(this).closest('.item-content'), id, name, group_id, true);
                ...
                if (activeProducts.length > 0) {
                    ...
                    activeProducts.each(function () {
                        ...
                        pr_items.each(function () {
                            ...
                            var arr = products.filter(function (el) {
                                return el.id !== undefined;
                            });

                            arr.length > 0 && arr.forEach(function (a) {
                                return pr.push(a);
                            });
                        });
                        products.push({ product_id: product_id, product_qty: product_qty, variations: variations });
        ...
        $("body").on('click', '#product-select-offer', function (e) {
            ...
            var variations = $("#special_offer_data").val() && JSON.parse($("#special_offer_data").val()).variations;

            console.log("$(#special_offer_data).val()", variations);
            $.ajax({
                type: "post",
        ...
        $("body").on('click', '.btn-add-to-cart', function (e) {
            ...
            var filter_discount_limit = null;
            ...
            variations.map(function (gr) {
                if (gr.limit) {
                    var limit = gr.limit * 1;
                    var qty = gr.products.reduce(function (el1, el2) {
