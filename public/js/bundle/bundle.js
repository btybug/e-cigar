...
$(document).ready(function () {
  ...
  $("#singleProductPageCnt").fadeIn(function () {
    ...
    var new_qty = function new_qty(group, type) {
      var qty = 0;
      if (type === 'popup') {
    ...
    var countPrices = function countPrices(modal) {
      ...
      $((modal ? '#extraModal' : '.single-product-dtls-wrap') + " [data-per-price]").each(function () {
        ...
        if ($this.attr('data-per-price') === 'product') {
          ...
          $this.closest('.product-single-info_row').find('.product-qty').length !== 0 && $this.closest('.product-single-info_row').find('.product-qty').each(function () {
            var $product_qty_input = $(this);
            var qty = Number($product_qty_input.val());
            var price = Number($product_qty_input.closest('[data-price]').attr('data-price'));
    ...
    var setTotalPrice = function setTotalPrice(modal) {
    ...
    var productsInit = function productsInit(modal) {
      var modalType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'all';

      var getParentId = modal ? '#extraModal' : '#requiredProducts';
      //--------------------------------select start
      ...
      var popupInit = function popupInit() {
        (function () {
          ...
          $(getParentId + " .popup-select").each(function () {
            var data_group_id = $(this).closest('.package_product').attr('data-group-id');
            var limit = 0;
            ...
            $("body").on('click', "#popUpModal[data-group=\"" + data_group_id + "\"] .modal-footer .b_save", function () {
              ...
              fetch("/products/get-variation-menu-raws", {
                ...
                var selected_product_wrapper = $("[data-group=\"" + data_group_id + "\"]").closest('.product-single-info_row').find('.product-single-info_row-items');
      ...
      var filterModalInit = function filterModalInit() {
        (function () {
          ...
          var makeSelectedItem = function makeSelectedItem(id, title) {
            return "<div class=\"col-md-2 col-sm-3 selected-item_popup\" data-id-popup=\"" + id + "\">\n                                          <div class=\"d-flex justify-content-between selected-item_popup-wrapper\">\n                                            <div class=\"align-self-center text-truncate\">\n                                              " + title + "\n                                            </div>\n                                            <div class=\"d-flex align-items-center justify-content-end\">\n                                              <div class=\"mr-1\">Qty</div>\n                                              <div class=\"continue-shp-wrapp_qty position-relative mr-0\">\n\n                                            <span class=\"d-flex align-items-center pointer position-absolute selected-item-popup_qty-minus qty-count\">\n                                            <svg viewBox=\"0 0 20 3\" width=\"12px\" height=\"3px\">\n                                            <path fill-rule=\"evenodd\" fill=\"rgb(214, 217, 225)\"\n                                            d=\"M20.004,2.938 L-0.007,2.938 L-0.007,0.580 L20.004,0.580 L20.004,2.938 Z\"></path>\n                                            </svg>\n                                            </span>\n                                            <input class=\"popup_field-input w-100 h-100 font-23 text-center border-0 selected-item-popup_qty-select\" min=\"number\" name=\"\"\n                                            type=\"number\" value=\"1\">\n                                            <span class=\"d-flex align-items-center pointer position-absolute selected-item-popup_qty-plus qty-count\">\n                                            <svg viewBox=\"0 0 20 20\" width=\"15px\" height=\"15px\">\n                                            <path fill-rule=\"evenodd\" fill=\"rgb(211, 214, 223)\"\n                                            d=\"M20.004,10.938 L11.315,10.938 L11.315,20.000 L8.696,20.000 L8.696,10.938 L-0.007,10.938 L-0.007,8.580 L8.696,8.580 L8.696,0.007 L11.315,0.007 L11.315,8.580 L20.004,8.580 L20.004,10.938 Z\"></path>\n                                            </svg>\n                                            </span>\n                                            </div>\n                                            <div>\n                                            <a href=\"javascript:void(0)\" data-el-id=\"" + id + "\" class=\"btn btn-sm delete-menu-item text-danger\"><i class=\"fa fa-times\"></i></a>\n                                            </div>\n                                            </div>\n                                            </div>\n                                        </div>";
          };

          $('.filters-modal-wizard').each(function () {
            var group_id = $(this).attr('data-group');
            var filter = [];

            $(this).on('click', function (e) {
              var first_category_id = $(this).attr('data-action');
              var self = $(this);
              var selectMoreItems = [];
              var selectSingleItems = void 0;
              ...
              $body.on('click', "#wizardViewModal[data-group=\"" + group_id + "\"] .add-items-btn", function (e) {
                ...
                if (Number(self.attr('data-multiple')) === 1) {
                  ...
                  selectMoreItems.forEach(function (item) {
              ...
              $.ajax({
                ...
                success: function success(data) {
                  if (!data.error) {
                    var modal_group_id = self.attr('data-group');
                    ...
                    if (!data.error) {
                      ...
                      if (data.type === "filter") {
                        ...
                        $("#wizardViewModal[data-group=\"" + group_id + "\"] .selected-item_popup").each(function () {
                          $(this).closest('#wizardViewModal').find(".wrap-item[data-id=\"" + $(this).attr('data-id-popup') + "\"]").length > 0 && $(this).closest('#wizardViewModal').find(".wrap-item[data-id=\"" + $(this).attr('data-id-popup') + "\"]").addClass('active');
                        });
                        $('.shopping-cart_wrapper .next-btn').addClass('d-none');
                        $('.shopping-cart_wrapper .add-items-btn').removeClass('d-none');
                      }
            ...
            $("body").on('click', ".filters-modal-wizard[data-group=\"" + group_id + "\"]", function () {
              ...
              $.ajax({
                ...
                success: function success(data) {
                  if (!data.error) {
                    $("#wizardViewModal .selected-items_filter").empty();
                    $(".filter[data-group-id=\"" + group + "\"]").closest('.product-single-info_row').find('.menu-item-selected').toArray().map(function (selectedItem) {
                      var selectedItemId = $(selectedItem).attr('data-id');
                      var selectedItemTitle = $(selectedItem).find('.delete-menu-item').parent().text().trim();
                      console.log('item', selectedItemId, selectedItemTitle);
                      $("#wizardViewModal .selected-items_filter").append(makeSelectedItem(selectedItemId, selectedItemTitle));
                    });
            ...
            $("body").on('click', "#wizardViewModal[data-group=\"" + group_id + "\"] .shopping-cart_wrapper .wrap-item", function (ev) {
              ...
              if (filter_limit > new_qty(null, 'filter') && !$(this).hasClass('active')) {
                $(this).addClass('active');
                $('.selected-items_filter').append(makeSelectedItem(id, title));
            ...
            $('body').on('click', '#wizardViewModal .selected-item_popup .delete-menu-item', function () {
              var remove_id = $(this).attr('data-el-id');
            ...
            $('body').on('click', "#wizardViewModal[data-group=\"" + group_id + "\"] .add-items-btn", function () {
              ...
              fetch("/products/get-variation-menu-raws", {
                ...
                $("[data-group=\"" + dg + "\"]").closest('.product-single-info_row').on('click', '.product-count-plus', function (ev) {
                  ...
                  ev.stopImmediatePropagation();
                  var input = $($(this).closest('.continue-shp-wrapp_qty').find('.field-input')[0]);
                  if (Number(input.val()) < Number(limit) - Number(new_qty($("[data-group=\"" + dg + "\"]"))) + Number($($(this).closest('.continue-shp-wrapp_qty').find('.field-input')[0]).val())) {
                    input.val(Number(input.val()) + 1);
                    var price = Number($(this).closest('[data-price]').attr('data-price'));
                    $(this).closest('[data-price]').find('.price-placee').html("$" + price * Number(input.val()));
      ...
      if (!modal && initCount === 0) {
        ...
        switch (modalType) {
          ...
          case 'list':
            ...
            break;
          default:
            selectInit();listInit();popupInit();filterModalInit();
        }
      }
    };
    ...
    $("body").on('click', '.btn-add-to-cart', function () {
      ...
      var item_validation = 0;
      $('#requiredProducts .limit').each(function (index, gr) {
        var $group = $(gr);
        var group_id = Number($group.attr('data-id'));
        var group_limit = Number($group.attr('data-limit'));
        var group_min_limit = Number($group.attr('data-min-limit'));
        var qty = 0;
      ...
      if (all_validation) {
        //
        var product_id = $('#vpid').val();
        var product_qty = $('.product-qty-select').val();
        var variations = $('#requiredProducts [data-group-id]').toArray().map(function (el) {
          var group_id = $(el).attr('data-group-id');
          var products = [];
          $("[data-group-id=\"" + group_id + "\"]").toArray().map(function (gr) {
            if ($(gr).closest('.product-single-info_row').find('.product-qty').length !== 0) {
...
window.AjaxCall = function postSendAjax(url, data, _success, _error4) {
  $.ajax({
    type: "post",
    url: url,
    cache: false,
      ...
      grecaptcha.execute(GOOGLE_RECAPTCHA_KEY, { action: 'action_name' }).then(function (token) {
        ...
        $.ajax({
          ...
          error: function error(_error5) {
            console.log($('form')[0]);
            errorHandler(firstNameEl, _error5.responseJSON.errors, _error5.responseJSON.errors.name, '#firstName');
            errorHandler(lastNameEl, _error5.responseJSON.errors, _error5.responseJSON.errors.last_name, '#lastName');
            errorHandler(emailEl, _error5.responseJSON.errors, _error5.responseJSON.errors.email, '#e-mail');
            errorHandler(phoneEl, _error5.responseJSON.errors, _error5.responseJSON.errors.phone, '#phoneNumber');
            errorHandler(passwordEl, _error5.responseJSON.errors, _error5.responseJSON.errors.password, '#password');
          }
        });
      });
    });
      ...
      grecaptcha.execute(GOOGLE_RECAPTCHA_KEY, { action: 'action_name' }).then(function (token) {
        ...
        $.ajax({
          ...
          error: function error(_error6) {
            ...
            errorHandler(emailEl, _error6.responseJSON.errors, _error6.responseJSON.errors.email, '#loginEmail');
            errorHandler(passwordEl, _error6.responseJSON.errors, _error6.responseJSON.errors.password, '#loginPass');
          }
        });
      });
    });
      ...
      grecaptcha.execute(GOOGLE_RECAPTCHA_KEY, { action: 'action_name' }).then(function (token) {
        ...
        $.ajax({
          ...
          error: function error(_error7) {
            var emailEl = $('#loginEmail');
            var passwordEl = $('#loginPass');
            errorHandler(emailEl, _error7.responseJSON.errors, _error7.responseJSON.errors.email, '#loginEmail');
            errorHandler(passwordEl, _error7.responseJSON.errors, _error7.responseJSON.errors.password, '#loginPass');
          }
        });
      });
    });
