        ...
        <div class="d-flex h-100">
            <div class="main-left-tabs d-flex flex-column kaliony-menu">
                <div class="nav flex-column justify-content-center nav-pills" id="v-pills-tab" role="tablist"
                    ...
                    @foreach($categories as $value)
                        <a class="nav-link {{ ($category && $category->id == $value->id) ? 'active' : '' }} d-flex flex-column align-items-center" id="v-pills-{{ $value->id }}-tab" data-toggle="pill"
                           href="#v-pills-{{ $value->id }}" role="tab" aria-controls="v-pills-{{ $value->id }}" aria-selected="true"><span
                                    class="sub txt-cl-red">{{ $value->name }}</span></a>
                    @endforeach
                </div>
            ...
            <div class="main-right-wrapp kaliony-page">
                <div class="tab-content" id="v-pills-tabContent">
                    @foreach($categories as $value)
                        <div class="tab-pane fade show {{ ($category && $category->id == $value->id) ? 'active' : '' }}" id="v-pills-{{ $value->id }}" role="tabpanel"
                             aria-labelledby="v-pills-hit-tab">
                            <div class="main-content">
                                <div class="kaliony-top d-flex flex-wrap">
                                    <div class="col-md-6">
                                        <div class="left-content">
                                            ...
                                            <div class="qualities">
                                                <ul class="d-flex flex-wrap list-unstyled justify-content-center">
                                                    @foreach($value->stickers as $sticker)
                                                    <li>
                                                        <div class="d-flex flex-wrap align-items-center">
                                                            <div class="icon">
                                                                @if($sticker->image)
                                                                    <img src="{{ $sticker->image }}" width="50" height="50"/>
                                                                @endif
                                                            </div>
                                                            <div class="title txt-cl-red">
                                                                {!! $sticker->name !!}
                                                            </div>
                                                        </div>
                                                    </li>
                                            ...
                                            <p class="info" style="min-height: 100px;">
                                                {!! $value->description !!}
                                            </p>
                                            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                                                @foreach($value->children as $item)
                                                    <li class="nav-item">
                                                        <a class="nav-link nav-link d-flex flex-column  {{ $loop->first ? 'active' : '' }}" id="tab-{{ $item->id }}"
                                                           data-toggle="tab" href="#subTab{{ $item->id }}" role="tab" aria-controls="subTab{{ $item->id }}" aria-selected="true">
                                    ...
                                    <div class="col-md-6 pr-0 ">
                                        <div class="image h-100">
                                            ...
                                            @else
                                                <img src="/public/images/no_image.jpg" alt="">
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                <div class="kaliony-bottom">
                                                                ...
                                                                <div class="col-lg-2 col-md-4 col-sm-6">
                                                                    <a href="javascript:void(0);" class="item">
                                                                        ...
                                                                        <div class="name text-center">
                                                                            <a href="{{ route('product_single_juice',[$value->slug,$product->id]) }}">{{ $product->name }}</a>
                                                                        </div>
                                                                    </a>
                                                                </div>
                                                            @endif
