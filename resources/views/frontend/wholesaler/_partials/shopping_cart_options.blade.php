...
@else
    @foreach(\Cart::session('wholesaler')->getContent() as $datum)
        @php
            $item = $datum->attributes->item;
        @endphp
        <!--Repeating cart item-->
            <div class="cart-sidebar_item text-sec-clr w-100 position-relative">
                <div class="row mb-3">
                    ...
                    <div class="col-7">
                        <h3 class="font-20 font-sec-bold">{!! $item->name !!}</h3>
                    </div>
                </div>
                <div class="row align-items-center">
                    <div class="col-5">
                        <div class="form-group mb-0">
                            ...
                            {!! Form::number('',$datum->quantity,['class' => 'cart-product-qty-select qty-input','min' => '1','data-uid' => $datum->id]) !!}
                        </div>
                    </div>
                    <div class="col-7">
                        <span class="d-block cart-product-price font-24 font-sec-bold cart-product-price">
                            {{ convert_price($datum->price,@$currency, false) }}
                        </span>
                    </div>
                </div>
                <a data-uid="{{ $datum->id }}" class="remove-from-cart cart-sidebar_item-close d-inline-block position-absolute pointer d-flex align-items-center justify-content-center">
